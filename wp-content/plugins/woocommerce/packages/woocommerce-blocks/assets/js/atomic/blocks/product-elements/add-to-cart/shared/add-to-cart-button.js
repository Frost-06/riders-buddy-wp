...
const AddToCartButton = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeProcessing =
      eventRegistration.onAddToCartAfterProcessingWithSuccess(onSuccess, 0);
    return () => {
   ...
   */
  const showButton =
    (showFormElements || (!productHasOptions && productType === "simple")) &&
    productIsPurchasable;
  ...
  return showButton ? (
    ...
    <LinkComponent
      ...
      text={addToCartButtonData.text || __("View Product", "woocommerce")}
    />
  );
};
    ...
    >
      {isDone && quantityInCart > 0
        ? sprintf(
            ...
            _n("%d in cart", "%d in cart", quantityInCart, "woocommerce"),
            quantityInCart
          )
        : __("Add to cart", "woocommerce")}
      {!!isDone && (
        <Icon srcElement={doneIcon} alt={__("Done", "woocommerce")} />
      )}
    </Button>
  );
};
