...
<div class="collapse wcfm-collapse" id="wcfm_products_manage">
	...
	<div class="wcfm-collapse-content">
		...
		<div class="wcfm-container wcfm-top-element-container">
			...
			if ($product_id) {
			?>
				<span class="product-status product-status-<?php echo $wcfm_products_single->post_status; ?>"><?php if ($wcfm_products_single->post_status == 'publish') {
				...
				<div id="wcfm_products_manage_form_general_expander" class="wcfm-content">
					<div class="wcfm_product_manager_general_fields">
						...
						$WCFM->wcfm_fields->wcfm_generate_form_field(apply_filters('wcfm_product_manage_fields_general', array(
							...
							"pro_title" => array('placeholder' => __('Product/Service Name', 'wc-frontend-manager'), 'type' => 'text', 'class' => 'wcfm-text wcfm_ele wcfm_product_title wcfm_full_ele simple variable external grouped booking', 'value' => $title),
					...
					<div class="wcfm_product_manager_gallery_fields">
						...
						if ($wcfm_is_allow_featured = apply_filters('wcfm_is_allow_featured', true)) {
							...
							$WCFM->wcfm_fields->wcfm_generate_form_field(apply_filters('wcfm_product_manage_fields_images', array(
								"featured_img" => array('type' => 'upload', 'class' => 'wcfm-product-feature-upload wcfm_ele simple variable external grouped booking', 'label_class' => 'wcfm_title', 'prwidth' => 250, 'value' => $featured_img),
						...
						<?php if (apply_filters('wcfm_is_category_checklist', true)) { ?>
							...
							if (apply_filters('wcfm_is_allow_tags', true)) {
								...
								if (apply_filters('wcfm_is_tags_input', true)) {
									$WCFM->wcfm_fields->wcfm_generate_form_field(apply_filters('wcfm_product_simple_fields_tag', array(
										"product_tags" => array('label' => __('Tags', 'wc-frontend-manager'), 'type' => 'textarea', 'class' => 'wcfm-textarea wcfm_ele wcfm_full_ele product_tags_ele simple variable external grouped booking', 'label_class' => 'wcfm_title wcfm_full_ele product_tags_ele', 'value' => $product_tags, 'placeholder' => __('Separate Product Tags with commas', 'wc-frontend-manager'), 'hints' => __('Product tags are descriptive labels you can add to your products. Popular search engines can use tags to get information about your store. You can add more than one tag separating them with a comma.', 'wc-frontend-manager'), 'desc' => __('Choose from the most used tags', 'wc-frontend-manager'), 'desc_class' => 'wcfm_full_ele wcfm_side_tag_cloud wcfm_fetch_tag_cloud')
									), $product_id, $product_type));
								} else {
									$product_all_tags = array();
									foreach ($product_defined_tags as $product_defined_tag) {
												...
												if ($product_taxonomy->public && $product_taxonomy->show_ui && $product_taxonomy->meta_box_cb && !$product_taxonomy->hierarchical) {
													if (apply_filters('wcfm_is_allow_custom_taxonomy_' . $product_taxonomy->name, true)) {
														...
														$WCFM->wcfm_fields->wcfm_generate_form_field(array(
															$product_taxonomy->name => array('label' => $product_taxonomy->label, 'name' => 'product_custom_taxonomies_flat[' . $product_taxonomy->name . '][]', 'type' => 'textarea', 'class' => 'wcfm-textarea wcfm_ele wcfm_full_ele simple variable external grouped booking', 'label_class' => 'wcfm_title wcfm_full_ele', 'value' => $taxonomy_values, 'placeholder' => __('Separate Product ' . $product_taxonomy->label . ' with commas', 'wc-frontend-manager'))
														));
													}
												}
											}
